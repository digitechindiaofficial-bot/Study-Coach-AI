...
- Pro plan: everything unlimited only while `planType` is "pro" and `planExpiry` is a valid future timestamp.
- Never expose a self-service API that lets a user write `planType`; only verified Razorpay payment handling or an admin-authorized grant may create an active Pro entitlement.
- `planType` defaults to "free"; expired, invalid, or missing `planExpiry` must be treated as Free by both client and server.
